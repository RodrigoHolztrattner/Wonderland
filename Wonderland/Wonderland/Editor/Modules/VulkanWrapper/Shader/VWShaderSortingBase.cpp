...
void VulkanWrapper::VWShaderRenderingBase::RenderOpaqueGeometry(VWContext* _graphicContext, int _swapchainImageIndex)
{
	...
	for (int i = 0; i < m_Renderables.size(); i++)
	{
		...
		// Get the texture identificator for the current object
		uint32_t textureIdentificator = GetTextureIdentificator(currentObject);

		// Check if the next object should be threated as a new object
		if (textureIdentificator != currentTextureIdentificator || currentObject->GetVertexObject()->GetId() != currentVertexObjectId)
		{
...
uint32_t VulkanWrapper::VWShaderRenderingBase::GetTextureIdentificator(VWRenderable* _instance)
{
	return 0;
}