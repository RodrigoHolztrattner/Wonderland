...
void VulkanWrapper::VWBufferClear::CreateCommandBuffers(VWGraphicAdapter* _adapter, VWGraphicInstance* _graphicInstance, VWSwapChain* _swapChain)
{
	...
	VkClearDepthStencilValue depth_clear_color =
	{ 1.0f, 0.0f};
	...
	VkImageSubresourceRange depth_image_subresource_range =
	{
	...
	for (size_t i = 0; i < commandBuffers.size(); i++)
	{
		...
		vkCmdClearDepthStencilImage(commandBuffers[i], _swapChain->GetDepthImage()->GetRawImage(), VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, &depth_clear_color, 1, &depth_image_subresource_range);
